...
output_switch_case (KeywordExt_List *list, int indent, int *jumps_away)
{
	...
	*jumps_away = 1;
    ...
    else if (option[DUP] && list->first()->_duplicate_link)
    {
	if (option[LENTABLE])
...
Output::output_lookup_function_body (const Output_Compare& comparison) const
{
	...
	else if(option[OUTPUTINDEX_S])
	{
	    printf ("          %schar* resword;\n\n          %sint index;\n\n",register_scs);
	}
	else if (option[TYPE])
	{
	    printf ("          %s",
...
Output::output ()
{
    ...
    if(option[OUTPUTINDEX] || option[OUTPUTINDEX_S])
    {
	_return_type = "const int";
	_struct_tag = "int";
    }
    else if (!option[TYPE])
    {
